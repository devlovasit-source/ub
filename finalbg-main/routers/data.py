...
def list_documents(
    ...
    try:
        ...
        requested_user_id = user_id or auth_user_id
        if requested_user_id:
            ensure_user_scope(user, requested_user_id)
        docs = proxy.list_documents(resource, user_id=requested_user_id, occasion=occasion, limit=limit)
...
def get_document(resource: str, document_id: str, user=Depends(get_current_user)):
    try:
        doc = proxy.get_document(resource, document_id)
        user_field = proxy.user_field_map.get(resource)
        if user_field:
            ensure_user_scope(user, str(doc.get(user_field, "")))
        return {"document": doc}
    except AppwriteProxyError as exc:
...
@router.post("")
def create_document(request: CreateRequest, user=Depends(get_current_user)):
    try:
        auth_user_id = user.get("user_id")
        ...
        if user_field and auth_user_id:
...
def update_document(document_id: str, request: UpdateRequest, user=Depends(get_current_user)):
    try:
        existing = proxy.get_document(request.resource, document_id)
        user_field = proxy.user_field_map.get(request.resource)
        if user_field:
            ensure_user_scope(user, str(existing.get(user_field, "")))
...
@router.delete("")
def delete_document(request: DeleteRequest, user=Depends(get_current_user)):
    try:
        existing = proxy.get_document(request.resource, request.document_id)
        user_field = proxy.user_field_map.get(request.resource)
        if user_field:
            ensure_user_scope(user, str(existing.get(user_field, "")))
...
def upsert_user_profile(user_id: str, body: Dict[str, Any], user=Depends(get_current_user)):
    try:
